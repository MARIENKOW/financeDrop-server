...
class Controller {
   ...
   signIn = async (req, res) => {
      try {
         ...
         if (!data) return res.status(400).json({email:'Email is not defined'})
         ...
         if (!dataValues.isActivated) return res.status(400).json({'root.server':'Account is not activated. check your email'})
   ...
   signUp = async (req, res) => {
      try {
         ...
         if (usernameFind) return res.status(400).json({ username:'Username is already taken' })

         const emailFind = await User.findOne({where:{email}})

         if (emailFind) return res.status(400).json({ email:'Email is already taken' })
   ...
   activate = async (req, res) => {
      try {
         ...
         if (!token) return res.status(400).json('Activation token is not found');

         const activate = await Activate.findOne({where:{
            token,
            date_end:{
         ...
         if (!activate) return res.status(400).json('Activation token is not found');
         await User.update({isActivated:1},{where:{id:activate.user_id}});
   ...
   rememberPassword = async (req, res) => {
      try {
         ...
         if (!userData) return res.status(400).json({ email:'Email is not found' })

         await RememberPass.destroy({where:{user_id:userData.id,dateEndChange:{[Op.lt]:sequelize.fn('NOW')}}})

         const rememberPassData = await RememberPass.findOne({where:{user_id:userData.id,dateEndChange:{[Op.gte]:sequelize.fn('NOW')}}})

         console.log(rememberPassData);
         if (rememberPassData) return res.status(400).json({ 'root.server':'The password reset email has already been send.' })

         const rememberPassLink = v4();

         const rememberPassDataNew = await RememberPass.create({rememberPassLink,user_id:userData.id});
         try {
            await mailService.sendMessage(userData.email, `${process.env.CLIENT_URL}/ChangePass/${rememberPassLink}`, 'change')
         } catch (error) {
   ...
   changePass = async (req, res,user_id) => {
      try {
         ...
         await User.update({password:hashPassword},{where:{id:req.user_id}});

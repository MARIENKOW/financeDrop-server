...
class ImgService {
   ...
   async moveFile(img,imgName) {
      return new Promise((res,rej) => {
         img.mv(path.resolve()+'/'+process.env.NFT_FOLDER +'/'+ imgName, function (err) {
   ...
   async unlinkFile(imgName) {
      return new Promise((res,rej)=>{
         unlink(path.resolve()+process.env.NFT_FOLDER+"/"+imgName,(err)=>{
            if(err) return rej(err)
            res(true)
         })
      })
   ...
   async delete(img_id) {
      ...
      const  {name:imgName,id} = await Img.findOne({where:{id:img_id}});

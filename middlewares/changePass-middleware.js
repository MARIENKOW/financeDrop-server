...
const changePassMiddleware = async (req, res, next) => {
   try {
      ...
      const rememberPassData = await RememberPass.findOne({where:{rememberPassLink}});

      if(!rememberPassData) return res.status(403).json('Token is not found')

      const rememberPassTimeData = await RememberPass.findOne({where:{rememberPassLink,dateEndChange:{[Op.gte]:sequelize.fn('NOW')}}});


      if(!rememberPassTimeData)  return res.status(403).json('Time to change pass is gone')
